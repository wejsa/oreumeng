...
- **`alt`가 사진마다 다름**: 관리자가 입력한 시공 장소 또는 간단한 설명을 저장 시 `alt`에도 자동 반영합니다. 별도의 사진 제목이나 대체 텍스트를 중복 입력받지 않습니다.
...
**현장 편집 화면**
- 현장명, 카테고리, 시공 설명, 공개 여부
- 사진 영역
  - 드래그&드롭 또는 파일 선택으로 다중 업로드
  - 업로드 전 미리보기 + 변환 진행률
  - 각 사진: 대표 지정(라디오), 순서 변경, **시공 장소 또는 간단한 설명(필수)**, 삭제
...
마이그레이션 단계에서 기존 카테고리명과 순번을 이용해 사진별 초기 `alt`를 자동 생성합니다(예: “크린부스 시공 사례 1”). 이후 관리자가 시공 장소 또는 간단한 설명을 수정하면 저장 시 해당 설명으로 `alt`도 자동 갱신합니다.
...
| 사진 설명이 지나치게 짧음 | 정보 전달·SEO 효과 반감 | 필수 입력 + 시공 장소가 포함된 예시 문구 표시(예: "안성 ○○공장 크린부스 내부 시공 모습") |
